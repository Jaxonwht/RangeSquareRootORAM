...
{
	if (argc != 6 && argc != 11) {
		printf("Call the binary as ./single parse <blk_size> <instructionfile> <logfile> <storagefile>\n");
	...
	if (!strcmp(argv[1], "parse") && argc == 6) {
		...
		gettimeofday(&tv1, NULL);
		oram = oram_init(blk_size, 1, blk_count, argv[5], NULL, 0);
		gettimeofday(&tv2, NULL);
		timediff = timediffusec(&tv1, &tv2);
		...
		fprintf(log_fp, "%ld\t", timediff);
		gettimeofday(&tv1, NULL);
		oram = oram_init(blk_size, 1, blk_count, argv[10], NULL, 0);
		gettimeofday(&tv2, NULL);
		timediff = timediffusec(&tv1, &tv2);
		fprintf(log_fp, "%d\t", blk_size);
		fprintf(log_fp, "%d\t", blk_count);
